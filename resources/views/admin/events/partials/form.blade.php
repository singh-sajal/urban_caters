<div class="grid md:grid-cols-2 gap-4">
    <div>
        ...
        <input type="text" name="title" value="{{ old('title', $event->title ?? '') }}" required class="field mt-1">
    </div>
    <div>
    ...
    <div>
        ...
        <input type="date" name="event_date" value="{{ old('event_date', optional($event->event_date ?? null)->format('Y-m-d')) }}" class="field mt-1">
    </div>
    <div>
        <label class="text-sm text-slate-600">Location</label>
        <input type="text" name="location" value="{{ old('location', $event->location ?? '') }}" class="field mt-1">
    </div>
</div>
<div>
    <label class="text-sm text-slate-600">Summary</label>
    <textarea name="summary" rows="3" class="field mt-1">{{ old('summary', $event->summary ?? '') }}</textarea>
</div>
<div>
    <label class="text-sm text-slate-600">Body</label>
    <textarea name="body" rows="5" class="field mt-1">{{ old('body', $event->body ?? '') }}</textarea>
</div>
<div class="grid md:grid-cols-2 gap-4">
    <div>
        ...
        @if(!empty($event->hero_image))
            <img src="{{ asset('storage/'.$event->hero_image) }}" class="mt-2 h-24 rounded-lg border border-slate-200">
        @endif
    </div>
    <div class="flex items-center space-x-2 mt-6 md:mt-10">
        <input type="checkbox" name="is_featured" value="1" @checked(old('is_featured', $event->is_featured ?? false)) class="h-4 w-4 rounded border-slate-400">
